...
const V = 1;
const KINDS = ['help_wanted', 'bid', 'award', 'cancel', 'settle'];
...
function foldThread(messages, opts = {}) {
  ...
  for (const { m, w } of rows) {
    ...
    const from = lower(m.from);
    {
      ...
      jobs.set(w.jobId, {
        ...
        bids: [], award: null, cancelled: null, settleClaims: [], settlement: null, at: orderOf(m),
      });
      continue;
    }
  }
  ...
  for (const { m, w } of rows) {
    ...
    if (w.kind === 'settle') {
      ...
      seenTx.add(w.txHash);
      const claim = { txHash: w.txHash, by: from, amountMicro: w.amountMicro, at: orderOf(m), verified: false };
      ...
      if (claim.verified && job.state === 'awarded') {
        job.state = 'settled';
        job.settlement = { txHash: w.txHash, from: job.requester, to: worker, amountMicro: w.amountMicro, blockTime: claim.blockTime, at: orderOf(m) };
      }
      continue;
    }
...
module.exports = { buildWork, parseWork, foldThread, jobsFrom, graphOf, settlementsFrom, mayApply, KINDS, MARKER, USDC_BASE };
