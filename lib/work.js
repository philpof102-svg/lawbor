...
/* Which of a job's declared dependencies this node has NEVER SEEN. A help_wanted posted with a
 * dependsOn on an unseen upstream is not an error, but the recipient will fold it as blockedByUnknown
 * until that upstream reaches them — so both write doors (HTTP /work and MCP) warn about it rather than
 * succeed silently. One function so the two doors cannot drift, which is a bug shape this repo has hit
 * more than once. */
function unseenDeps(messages, dependsOn) {
  if (!Array.isArray(dependsOn) || !dependsOn.length) return [];
  const known = foldThread(messages);
  return dependsOn.filter((d) => !known.has(d));
}

module.exports = { buildWork, parseWork, foldThread, jobsFrom, graphOf, settlementsFrom, provenFrom, priceNumber, mayApply, unseenDeps, KINDS, MARKER, USDC_BASE };
