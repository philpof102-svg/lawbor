...
function createPaywall(cfg = {}) {
  ...
  return {
     ...
     * we record a subscription for the payer.
     *
     * Fail-closed on: no verifier · a throw · a proof that does not verify · a payer that is not an
     * address · an amount that is UNREADABLE · an amount below the price · a recipient the verifier
     * reports and that is not `payTo`.
     * ⚠️ Cette liste disait « wrong recipient » sans que rien ne compare quoi que ce soit a `payTo`.
     * Le destinataire n'est verifie QUE si le verifieur le rapporte — son contrat documente ne le porte
     * pas — et `recipientChecked` dans la reponse dit lequel des deux cas s'est produit. Une garantie
     * qu'on ne peut pas tenir seule se DECLARE, elle ne se promet pas.
    ...
    async settle(paymentPayload) {
      ...
      /* ⛔ UN MONTANT ILLISIBLE N'EST PAS UN MONTANT SUFFISANT. La garde etait
       *     if (Number(v.amountUsdc) + 1e-9 < Number(price))
       * et `Number(undefined)` vaut NaN, or TOUTE comparaison avec NaN est fausse — donc la garde ne
       * se declenchait pas et l'abonnement etait accorde. Mesure du 2026-08-15, prix 5 USDC:
       *
       *   amountUsdc '5'        -> refus: non, abonnement actif   (correct)
       *   amountUsdc '1'        -> REFUSE « underpaid »           (correct)
       *   amountUsdc undefined  -> ok:true, ABONNEMENT ACTIF      <<< 30 jours pour un montant jamais lu
       *   amountUsdc 'abc'      -> ok:true, ABONNEMENT ACTIF      <<< idem
       *   amountUsdc null       -> REFUSE (Number(null) === 0)
       *
       * 💎 Les DEUX formes d'absence se comportent a l'OPPOSE: `null` devient 0 et ferme la porte,
       * `undefined` devient NaN et l'ouvre. Le lire d'abord, comparer ensuite. */
      const paid = Number(v.amountUsdc);
      if (!Number.isFinite(paid)) {
        return { ok: false, reason: 'the verifier reported no readable amount (' + JSON.stringify(v.amountUsdc)
          + ') — FAIL CLOSED: an amount we cannot read is never an amount that was paid' };
      }
      if (paid + 1e-9 < Number(price)) return { ok: false, reason: 'underpaid: ' + v.amountUsdc + ' < ' + price + ' USDC' };
      /* LE DESTINATAIRE. Le commentaire de cette fonction promettait « wrong recipient => no
       * subscription » et RIEN n'etait jamais compare a `cfg.payTo` — une garantie attribuee au
       * verifieur injecte, dont le contrat documente (`{ok, payer, amountUsdc}`) ne porte meme pas le
       * destinataire. On ne peut donc pas l'exiger sans casser les verifieurs existants.
       * ⚖️ Ce qu'on fait a la place, et qui est honnete: si le verifieur le rapporte, un ECART est
       * refuse; s'il ne le rapporte pas, on ne PRETEND pas avoir verifie — `recipientChecked` porte la
       * difference jusqu'a l'appelant, au lieu de la laisser dans un commentaire. */
      const reporte = v.payTo || v.recipient || v.to || null;
      if (reporte != null && lower(reporte) !== lower(cfg.payTo)) {
        return { ok: false, reason: 'paid the WRONG recipient — the verifier reports ' + lower(reporte)
          + ', this paywall collects at ' + lower(cfg.payTo) };
      }
      const until = clock() + periodMs;
      cfg.subs.record(lower(v.payer), until);
      return { ok: true, payer: lower(v.payer), until, recipientChecked: reporte != null };
    },
  };
}
