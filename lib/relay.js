 ...
 *     ⚠️ ET C'EST LUI QUI EMPECHE LA BOUCLE INFINIE, pas le plafond de hops. Mesure du 2026-08-15,
 *     trois cas: (a) relais neuf, hops=7 > maxHops=6 -> drop « hop cap exceeded », le plafond marche;
 *     (b) MEME relais, un relais hostile ayant remis hops a 0 -> drop « already seen (dedup) »;
 *     (c) relais qui ne l'a JAMAIS vue, hops remis a 0 -> FORWARD. `hops` n'est ni signe ni couvert
 *     par l'id — il ne PEUT pas l'etre, chaque relais l'incremente — donc n'importe qui le remet a
 *     zero. Le plafond borne donc UN CHEMIN TEL QU'IL SE DECLARE, jamais la circulation. Ce qui la
 *     borne est le dedup: chaque relais ne laisse passer un id qu'une fois, donc une enveloppe
 *     traverse au plus N relais dans un mesh de N, quel que soit hops.
 *     ⛔ CONSEQUENCE POUR `seen`: son absence de borne (voir plus bas) ne porte pas seulement le
 *     dedup, elle porte CETTE garantie. Un Set injecte qui EVINCE rend a une enveloppe un second
 *     passage sur le meme relais — et la circulation infinie redevient possible. Le plafond ne
 *     rattraperait rien: l'attaquant remet hops a 0. Epingle par un test.
 *   - HOP CAP: bounds the length of a path AS DECLARED (an honest chain stops at maxHops).
...
function createRelay(cfg = {}) {
  ...
  /* DEDUP — et cet ensemble N'EST PAS BORNE, par dessein. Il grandit d'une entree par enveloppe
   * (accept + originate), pour la duree du processus.
   *
   * ⚖️ C'est le choix CORRECT et il ne doit pas etre "durci": evincer une entree ferait re-livrer une
   * vieille enveloppe au premier rejeu de gossip, ce qui est exactement ce que le dedup existe pour
   * empecher. La correction exige de se souvenir; le cout est la memoire, pas la surete.
   *
   * ⚠️ CE QUI MANQUAIT: le DIRE. `lib/mesh.js` borne son memo d'admission explicitement (`maxMemo`,
   * « bounded: drop oldest ») — meme depot, meme classe de structure — et ici rien n'annoncait la
   * croissance ni ne permettait de l'observer. Un operateur qui fait tourner un relais des mois n'avait
   * aucun signal, et aucune indication que `cfg.seen` EST le levier prevu: injecter un Set borne, un
   * store persistant, ou une structure a expiration est la façon supportee de choisir une autre
   * politique. La borne voyage desormais avec la mesure via `seenCount` ci-dessous.
   *
   * ⛔ Un Set injecte qui EVINCE rend le dedup approximatif: c'est un arbitrage d'operateur (memoire
   * contre re-livraison possible), assume par celui qui l'injecte, jamais par defaut ici. */
  ...
  /* PROBATION — an OPT-IN operator policy, default OFF, that fixes a real onboarding wall.
   * ------------------------------------------------------------------------------------
   * MainStreet only scores addresses it has already INDEXED: an unknown wallet gets score `null`, not 0.
   * With PROCEED as a hard requirement, LAWBOR's mesh is therefore closed to anyone MainStreet has never
   * seen — measured on four of the operator's own wallets, one with 2241 transactions, all refused. A
   * newcomer cannot build the history that admission requires without first being admitted. That kills
   * onboarding, which kills the network.
   *
   * Why this is now SAFE to offer, when it was not when the PROCEED rule was written: the protection has
   * moved. A stranger's standing is 0 by CONSERVATION (lib/credit.js) — they cannot farm reputation no
   * matter how much they talk or how much they wash on-chain — and the consent gate (lib/consent.js)
   * quarantines first contact anyway. The mesh gate was doing a job two better layers now do.
   *
   * What probation does NOT do: it never grants standing, never bypasses consent, and never reaches a
   * human inbox on its own (the caller quarantines it — see `probation` on the delivery). It only lets a
   * stranger SPEAK, which is the one thing they must be able to do to ever earn anything.
   * Default stays proceed-only: an operator opts in deliberately. */
  const admitProbation = cfg.admitProbation === true;

  async function gate(sender) {
    /* THE ORACLE OUTAGE, and why probation must survive it.
     * ------------------------------------------------------------------------------------------
     * This used to fail closed unconditionally: one unreachable HTTP service and EVERY node refused
     * EVERY envelope, everywhere. For a system whose entire claim is that no authority decides who
     * exists, that made one service we happen to operate a single point of failure AND of control.
     * Phil's question — "et si on rend chaque nœud décentralisé directement ?" — lands exactly here.
     *
     * The fix is not "fail open", which would gut the gate for operators who rely on the score. It is
     * that an outage must be mapped onto a decision THE OPERATOR HAS ALREADY MADE:
     *   - proceed-only (the default): no score means no decision. Still refuse. Unchanged.
     *   - admitProbation: the operator has already declared that an unknown or low-scoring stranger is
     *     admitted at score 0. An outage tells us strictly LESS than a CAUTION answer, and CAUTION is
     *     already admitted — so refusing on an outage contradicts their own configuration.
     * Soundness: this creates NO NEW STATE. An outage-admitted sender is byte-identical to one whose
     * oracle answered CAUTION — score 0, probation:true, consent still quarantines them, and
     * conservation means their standing is 0 no matter what they do. The protection was never here. */
    let v;
    try { v = await cfg.preflight(sender); }
    catch (e) {
      if (!admitProbation) return { ok: false, reason: 'preflight down — FAIL CLOSED (' + e.message + ')' };
      return { ok: true, score: 0, probation: true, decision: 'ORACLE-DOWN',
        reason: 'oracle unreachable (' + e.message + ') — admitted ON PROBATION, which is what this operator already grants an unknown sender' };
    }
    const proceed = !!v && v.decision === 'PROCEED' && Number(v.score) >= minScore;
    if (proceed) return { ok: true, score: v.score, probation: false };
    if (!admitProbation) {
      /* ⛔ NE NOMME PLUS UN ORACLE EN DUR. `preflight` est INJECTABLE par conception — c'est tout
       * l'interet de `deps.preflight`, et c'est ce qui permet a un operateur de remplacer notre oracle
       * par un autre. Ce message disait « on MainStreet » quoi qu'il arrive: mesure le 2026-08-09 avec
       * un preflight gitlawb injecte, un expediteur refuse recevait
       * « sender not PROCEED on MainStreet (NO-BINDING) » — donc il serait alle deboguer chez un
       * service qui n'y etait pour rien. Le refus nomme desormais la source QUI A REPONDU quand elle se
       * declare, et reste muet sur l'identite sinon. */
      if (!v || v.decision !== 'PROCEED') {
        const ou = v && typeof v.source === 'string' && v.source ? ' [' + v.source + ']' : '';
        return { ok: false, reason: 'sender not PROCEED per the configured admission oracle'
          + ou + ' (' + ((v && v.decision) || 'UNKNOWN') + ')' };
      }
      return { ok: false, reason: 'sender score ' + (v && v.score) + ' < ' + minScore + ' — too low to relay' };
    }
    // admitted ON PROBATION: score is treated as 0 (never the oracle's null), and the flag travels with
    // every delivery so nothing downstream can mistake a probationer for a vouched-for peer.
    return { ok: true, score: 0, probation: true, decision: (v && v.decision) || 'UNKNOWN' };
  }

  return {
    self, minScore, maxHops, authenticates: typeof cfg.verifySig === 'function',
    /* La croissance du dedup, OBSERVABLE. Sans ce compteur, un ensemble non borne grandit sans que
     * personne puisse le constater — et une croissance qu'on ne peut pas mesurer ne se decide pas.
     * Getter: il lit la taille au moment de l'appel, jamais un instantane fige a la construction. */
    get seenCount() { return seen.size; },
    seenIsBounded: cfg.seen ? null : false,   // false = le Set par defaut, non borne · null = injecte, la politique appartient a l'operateur
    peers: peerList,
    ...
    async accept(env) {
      ...
      if (env.to.toLowerCase() === self) {
        ...
        // `probation` travels with the delivery so the caller can never hand a probationer straight to a
        // human inbox — it is quarantined like first contact, whatever the consent list says.
        return { action: 'deliver', to: 'human', envelope: env, senderScore: g.score, authenticated: a.authenticated, probation: !!g.probation };
      }
