...
/* ⛔ CE LECTEUR PRENAIT LE PREMIER NOMBRE DE LA CHAINE ET JETAIT TOUT LE RESTE — L'UNITE COMPRISE.
 * `decideAward` s'en sert pour DEUX choses: le plafond `maxPrice` et le tri « le moins cher gagne ». Un
 * bot autonome ENGAGE donc de l'argent sur ce classement. Mesure du 2026-08-15, maxPrice 100:
 *
 ...
 *   « 25 USDC » vs « v2 rewrite, 200 USDC » -> AWARD a 200 USDC  (le premier nombre lu est le 2 de « v2 »)
 *
 * Le troisieme n'a rien d'exotique: « v2 » dans un libelle de prix est un texte que n'importe qui ecrit.
 *
 * ⚖️ DEUX REGLES, ET AUCUNE N'INVENTE DE POLITIQUE. (1) Un prix se lit ANCRE — un nombre, une unite
 * optionnelle, rien d'autre — sinon il est ILLISIBLE, jamais devine. (2) Une unite explicite doit etre
 * celle que ce bot compare (`currency`, deja dans DEFAULTS et deja utilisee pour FORMER nos propres
 * offres): comparer deux grandeurs d'unites differentes ne les classe pas, ca les melange. Un nombre nu
 * reste accepte et vaut la devise du bot — c'est la forme que `decideBid` produit lui-meme.
 * ⛔ Choisir QUELLES devises accepter reste un arbitrage d'operateur: on refuse de COMPARER, on ne
 * decrete pas qu'une devise est mauvaise. */
const priceOf = (s, currency) => {
  const m = String(s == null ? '' : s).trim().match(/^(-?\d+(?:\.\d+)?)\s*([A-Za-z]{1,10})?$/);
  if (!m) return NaN;                                   // pas un prix: illisible, jamais devine
  const unit = m[2] ? m[2].toUpperCase() : null;
  const veut = currency ? String(currency).toUpperCase() : null;
  if (unit && veut && unit !== veut) return NaN;         // autre unite: incomparable, pas « moins cher »
  return Number(m[1]);
};
...
function decideAward(job, self, policy = {}) {
  ...
  /* `prix` porte la devise du bot: une offre illisible ou libellee dans une AUTRE unite rend NaN et
   * sort du lot. Elle n'est pas « chere », elle est INCOMPARABLE — et le refus le dit separement, sinon
   * un operateur lit « no bid within maxPrice » et baisse son plafond pour un probleme d'unite. */
  const prix = (s) => priceOf(s, p.currency);
  const lisibles = job.bids.filter((b) => Number.isFinite(prix(b.price)));
  const usable = lisibles.filter((b) => prix(b.price) <= p.maxPrice);
  if (!usable.length) {
    return { award: false, reason: lisibles.length
      ? 'no bid within maxPrice'
      : `no COMPARABLE bid: ${job.bids.length} offer(s), none readable as a number in ${String(p.currency || '?').toUpperCase()} — an offer in another unit is not cheaper, it is incomparable` };
  }
  const byPrice = usable.slice().sort((a, b) => (prix(a.price) - prix(b.price)) || (a.at - b.at));
  ...
  let best = byPrice[0];
  if (tol > 0) {
    const ceiling = prix(byPrice[0].price) * (1 + tol);
    const within = byPrice.filter((b) => prix(b.price) <= ceiling);
    best = within.slice().sort((a, b) => (standing(b.worker) - standing(a.worker))
      || (prix(a.price) - prix(b.price)) || (a.at - b.at))[0];
  }
  return {
