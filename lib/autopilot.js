...
const DEFAULTS = {
   ...
   * `credit` is INJECTED (a Map addr→USDC-micro this node itself has verifiably paid them), because
   * computing it needs chain I/O and this file must stay pure. */
  credit: null,
...
function decideBid(job, self, jobs, policy = {}) {
  ...
  // What has this requester actually PAID US, verified on Base? Not a score, not a reputation someone
  // else vouched for — our own settled history with them. That is the only question a worker deciding
  // whether to take a job can answer without trusting a third party.
  const paidUs = p.credit ? Number((p.credit.get ? p.credit.get(job.requester) : p.credit[job.requester]) || 0) : 0;
