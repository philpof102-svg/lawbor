 ...
 * report. (A block stops future messages; store.deleteMsg now also scrubs one already stored.) The
 * reputation gate answers "may this bot relay into the MESH?"; it does NOT answer
...
function decideInbound(m = {}) {
  ...
  // block wins over EVERYTHING, including bot origin — a block is total, not just an inbox filter.
  if (blocked.has(from)) return { bucket: 'blocked' };
  if (m.origin !== 'human') return { bucket: 'bot' };       // the autonomous watch feed (not quarantined)
