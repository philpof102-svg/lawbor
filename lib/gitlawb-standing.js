...
/**
 * verifierNoeud — le nœud au bout de l'URL est-il celui qu'on croit ?
 *
 * 💎 SONDÉ LE 2026-08-09 SUR `https://node.gitlawb.com`, qui se déclare lui-même:
 *
 *     { auth: 'http-signature-rfc9421', did: 'did:key:z6Mki…', identity: 'ed25519',
 *       name: 'gitlawb-node', network: 'alpha', p2p_peer_id: '12D3KooW…',
 *       protocols: ['git-smart-http','mcp','libp2p'], version: '0.7.0' }
 *
 * ⛔ UNE URL N'EST PAS UNE IDENTITÉ. Un DNS détourné, un proxy, un opérateur qui change de main: tout
 * ça laisse l'URL intacte. Le nœud, lui, publie son **DID**. Épingler ce DID transforme « je fais
 * confiance à une adresse » en « je fais confiance à une identité », et rend un endpoint substitué
 * DÉTECTABLE au lieu d'être silencieux — dans une porte d'admission, la différence est tout.
 *
 * ⚠️ CE QUE CETTE FONCTION NE FAIT PAS: elle LIT ce que le nœud déclare, elle ne le PROUVE pas. Le nœud
 * annonce `auth: http-signature-rfc9421`, donc une vérification forte consisterait à valider une
 * signature HTTP contre ce DID. Tant que ce n'est pas fait, ceci reste un contrôle de COHÉRENCE:
 * il attrape une substitution maladroite, pas un adversaire qui rejoue la bonne réponse.
 *
 * ⚠️ ET IL REND `network` SANS LE JUGER. Le nœud public se déclare **`alpha`**. Une décision
 * d'admission bâtie dessus hérite d'une donnée de réseau alpha, et l'opérateur doit le voir.
 *
 * @returns {{ok:boolean, did:string|null, network:string|null, raison:string}}
 */
function verifierNoeud(carte, { didAttendu = null } = {}) {
  if (!carte || typeof carte !== 'object') {
    return { ok: false, did: null, network: null,
      raison: 'le noeud n a rien rendu de lisible — on ne sait pas a qui on parle' };
  }
  const did = typeof carte.did === 'string' ? carte.did : null;
  const network = typeof carte.network === 'string' ? carte.network : null;
  if (!did) {
    return { ok: false, did: null, network,
      raison: 'ce noeud ne publie pas de DID: son identite ne peut pas etre epinglee, seule son URL le designe' };
  }
  if (didAttendu && did !== didAttendu) {
    return { ok: false, did, network,
      raison: 'DID INATTENDU — l URL repond mais l identite a change: ' + did + ' au lieu de ' + didAttendu };
  }
  return { ok: true, did, network,
    raison: didAttendu ? 'identite conforme au DID epingle' + (network ? ' (reseau ' + network + ')' : '')
      : 'DID lu mais AUCUN DID attendu n a ete fourni — rien n est epingle, seule la coherence est visible'
        + (network ? ' (reseau ' + network + ')' : '') };
}

module.exports = { parseTrust, parseResolve, makeLireStanding, verifierNoeud, LECTURES_AUTORISEES };
