...
function createNode(cfg = {}) {
  ...
  return {
    ...
    async receive(env) {
      ...
      if (r.action === 'deliver') {
        // it reached its human. origin = the peer's intent: a human-authored msg (viaHuman set) vs a bot's autonomous msg.
        store.record(env, { origin: env.viaHuman ? 'human' : 'bot', dir: 'in', senderScore: r.senderScore });
