...
function createNode(cfg = {}) {
  ...
  return {
    ...
    async receive(env) {
      ...
      if (r.action === 'deliver') {
        /* CONSENT gate — reputation decided this bot may RELAY into the mesh; consent decides whether
         * this human-origin message reaches ME. A blocked sender is dropped BEFORE the append-only
         * store records anything (there is no delete, so a stored abusive body is permanent), and we
         * return a plain drop — no delivery confirmation — so a blocked sender cannot tell a block
         * from silence. Bot-origin (autonomous watch feed) is never quarantined and never blocked
         * here. Without this seam lib/consent.js would be decorative — the repo's documented failure
         * mode (a reviewed-green layer nothing wires). */
        if (env.viaHuman) {
