 ...
 */
const { buildEnvelope, signablePayload } = require('./envelope');
...
function createNode(cfg = {}) {
  ...
  /* Dispatch to every target and REPORT WHAT HAPPENED, per target.
   *
   * This used to swallow every outcome — `for (…) { try { await cfg.send(…) } catch {} }` — and say()
   * then reported `delivered: r.action === 'forward'`, a value decided BEFORE dispatch even ran. So a
   * caller was told `delivered:true` while the peer's node had answered 202 {action:'drop'} and stored
   * nothing. An external tester hit exactly that on a cross-node probe and was right to flag it: it is
   * the one failure this project treats as worse than an error, a field claiming more than it knows.
   *
   * Three outcomes, because there genuinely are three. A transport that returns nothing (an injected
   * one, or an older adapter) yields `null` — UNKNOWN — and null must never be collapsed into false:
   * "we could not find out" and "it was refused" are different facts, and guessing between them is how
   * the original bug read as reasonable. */
  async function dispatch(env, targets) {
    const out = [];
    for (const to of targets) {
      try {
        const r = await cfg.send(to, env);
        // A transport may report {ok, reason}; one that reports nothing leaves us honestly unsure.
        if (r && typeof r === 'object' && 'ok' in r) out.push({ to, ok: !!r.ok, reason: r.reason || null });
        else out.push({ to, ok: null, reason: 'transport does not report acceptance' });
      } catch (e) {
        // Still retriable — dedup makes a resend safe — but it is NOT a delivery, and we now say so.
        out.push({ to, ok: false, reason: (e && e.message) || 'send failed' });
      }
    }
    return out;
  }

  /** Fold per-target outcomes into the one field a caller reads. null (unknown) survives on purpose. */
  function deliveredFrom(action, results) {
    if (action !== 'forward') return false;                       // our own gate did not forward it
    if (results.some((r) => r.ok === true)) return true;          // at least one peer took it
    if (results.some((r) => r.ok === null)) return null;          // nobody confirmed, nobody denied
    return false;                                                 // every target refused or failed
  }

  /* ORIGINATION, the missing half of the pair.
   * ================================================================================================
   * The relay has verified `from` for weeks (injected verifySig, fail-closed) — but nothing ever SIGNED
   * an outbound envelope. say()/botSay() built one and dispatched it immediately, so two honest nodes
   * could never talk: A sends unsigned, B fail-closes. In practice the whole mesh only worked with
   * allowUnauthenticated, which the relay itself calls a spoofable `from`. Verification was built;
   * origination was not.
   *
   * `sign` is INJECTED, exactly like preflight / verifySig / chain / x402verify — ten such seams already
   * exist and this was the one gap. The node still holds NO KEY: sign() is the operator's own function,
   * wired to their wallet, KMS or hardware. We deliberately do NOT ship a "put your private key in an
   * env var" adapter — that would hand the key to the node and break the one promise this project makes
   * on every surface it has.
   *
   * 🛑 NEVER DOWNGRADE SILENTLY. If a signer is configured and then fails, the message is REFUSED, not
   * sent unsigned. A silent fallback is the worst possible outcome: the operator believes every envelope
   * carries their signature while the mesh quietly accepts spoofable ones. No signer at all is a
   * different, honest state — unsigned, and the receiving relay is free to fail-closed on it. */
  async function attachSignature(envelope) {
    if (typeof cfg.sign !== 'function') return { refused: false };   // unsigned by design, not by accident
    let sig;
    try { sig = await cfg.sign({ payload: signablePayload(envelope), envelope }); }
    catch (e) { return { refused: true, reason: 'signer threw — REFUSED rather than sent unsigned (' + ((e && e.message) || e) + ')' }; }
    if (typeof sig !== 'string' || !/^0x[0-9a-fA-F]+$/.test(sig)) {
      return { refused: true, reason: 'signer returned no usable signature — REFUSED rather than sent unsigned' };
    }
    envelope.sig = sig;
  ...
  return {
    ...
    /** Is this node able to produce signed envelopes at all? Reported honestly, like relay.authenticates. */
    originatesSigned: typeof cfg.sign === 'function',
    ...
    async say(to, body, opts = {}) {
      ...
      const signed = await attachSignature(envelope);
      if (signed.refused) return { envelope, sign, delivered: false, reason: signed.reason };
      store.record(envelope, { origin: 'human', dir: 'out' });
      const r = await relay.originate(envelope);
      const results = r.action === 'forward' ? await dispatch(r.envelope, r.targets) : [];
      // forwarded = OUR gate agreed. delivered = a PEER actually took it. They are different questions,
      // and reporting the first as the second is what made a cross-node probe read as a success.
      return { envelope, sign, forwarded: r.action === 'forward', delivered: deliveredFrom(r.action, results), targets: results, reason: r.reason || null };
    },
    ...
    async botSay(to, body, opts = {}) {
      ...
      const r = await relay.originate(envelope);
      const results = r.action === 'forward' ? await dispatch(r.envelope, r.targets) : [];
      return { envelope, sign, forwarded: r.action === 'forward', delivered: deliveredFrom(r.action, results), targets: results };
    },
    ...
    /* The zero-fallback here was the same lie store.js:109 told: an injected store that CANNOT compact
     * answered "I compacted and removed nothing". Nulls + a state say what actually happened, matching
     * deleteMsg's `{ ok:false, reason:'unsupported' }` right above. */
    compact(o) {
      return typeof store.compact === 'function' ? store.compact(o)
        : { totalBefore: null, kept: null, removed: null, state: 'unsupported',
            detail: 'this store has no compact() — nothing was read and nothing was removed' };
    },
    /* Bid GC — drop bid messages that can no longer change any outcome (a resolved
     * job's losing bids; optionally a stranded blocked-job bid past bidTtlMs). Fold
     * decides eligibility (work.staleBidIds); removal reuses the same operator-local
     * tombstone+compact path as deleteMsg — no key, no gossip, no funds. Off by
     * default for the stranded class (bidTtlMs:0) so it only reaps provably-dead bids. */
    gcBids(o = {}) {
      const ids = require('./work').staleBidIds(store.all(), { now: o.now, bidTtlMs: o.bidTtlMs || 0 });
      let removed = 0;
      for (const id of ids) { const r = store.deleteMsg(id); if (r && r.ok) removed++; }
      if (o.compact !== false && removed > 0 && typeof store.compact === 'function') store.compact({ now: o.now });
      return { eligible: ids.length, removed };
    },
  };
}
