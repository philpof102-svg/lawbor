 ...
 * @param {object} o
 * @param {number} o.now
 * @param {(a:{message:string, attestation:object})=>boolean} [o.verifier]  injecté; son absence => bound:false
 * @returns {{bound:boolean, did:string|null, address:string|null, message:string|null, raison:string, reVerify:string}}
 */
function jugerLiaison(att, { now = 0, verifier = null } = {}) {
  ...
  let ok = false;
  try { ok = verifier({ message, attestation: att }) === true; }
  catch (e) {
...
function makeLireLiaison(deps = {}) {
  ...
  return async function lireLiaison(addr) {
    ...
    return jugerLiaison(att, { now: horloge(), verifier });
  };
}
