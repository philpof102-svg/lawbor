 ...
 * @param {object} o
 * @param {number} o.now  horloge en ms — OBLIGATOIRE des que l'attestation porte une expiry.
 *        « Aucune horloge implicite » veut dire: sans horloge fournie, « pas perimee » est
 *        INDECIDABLE et on REFUSE — jamais un jugement rendu avec l'an 1970. Mesure du 2026-08-16:
 *        l'ancien defaut `now = 0` faisait exactement ca — une attestation expiree en nov. 2023,
 *        jugee sans opts, rendait `bound: true, "liaison bidirectionnelle verifiee"`. La chaine de
 *        production etait saine (makeLireLiaison injecte Date.now); le piege ne touchait que le
 *        consommateur DIRECT de cet export npm — celui dont les tests d'ici fournissaient `now: 1`
 *        a sa place, le motif « la fixture fournit ce que l'appelant reel omet ».
 ...
 */
async function jugerLiaison(att, { now, verifier = null } = {}) {
  ...
  if (expiry > 0) {
    /* ⛔ PAS D'HORLOGE N'EST PAS « PAS PERIMEE ». Sans `now` fourni, ce bloc jugeait avec l'ancien
     * defaut 0 (l'an 1970): `0 >= expiry` n'est jamais vrai, donc une attestation EXPIREE sortait
     * `bound: true` des que l'appelant omettait l'option — le fail-open exact que l'expiry existe
     * pour empecher. Purete conservee: on n'appelle toujours aucune horloge ici; on refuse de juger
     * ce qu'on ne peut pas decider. Une attestation SANS expiry reste jugeable sans horloge. */
    const horloge = Number(now);
    if (!Number.isFinite(horloge) || horloge <= 0) {
      return { bound: false, did, address, message, reVerify,
        raison: 'attestation avec expiry mais AUCUNE horloge fournie (opts.now) — sans horloge, '
          + '« pas perimee » est indecidable; on refuse de juger plutot que de juger avec l an 1970' };
    }
    if (horloge >= expiry) {
      return { bound: false, did, address, message, reVerify,
