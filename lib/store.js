...
function createStore(file = FILE, controlFile, opts = {}) {
  ...
  /* ⛔ « two node processes on one LAWBOR_DB would desync » — la ligne ci-dessus l'annonce, et rien ne
   * la rattrapait. `_mut` ne bouge que sur NOS propres ecritures: il ne peut pas voir ce qu'un AUTRE
   * processus fait au meme fichier. Une fois amorce, ce cache repondait pour toujours ce qu'il avait lu.
   *
   * MESURE DU 2026-08-16, deux instances de store sur un fichier — et c'est LE cas qui compte, parce
   * que la suppression existe ici « for a harassment victim »:
   *     P1 deleteMsg('M1') puis compact()  -> le corps QUITTE le disque (verifie absent du fichier)
   *     P2 relit                           -> rend encore le corps supprime, MOT POUR MOT
   * La suppression n'avait pas eu lieu pour le second lecteur, et rien ne l'aurait jamais rattrapee:
   * un lecteur qui n'ecrit pas ne bump jamais `_mut`.
   *
   * On retient donc la TAILLE que le cache reflete, et une lecture qui la voit changer re-amorce. Un
   * statSync coute des microsecondes; relire tout le journal a chaque lecture est exactement ce que ce
   * cache existe pour eviter. Le compteur suit nos ecritures locales (append = +N octets, compact = la
   * nouvelle taille), donc nos propres ecrits ne declenchent PAS de re-amorcage inutile. */
  let cacheBytes = null;          // taille du fichier que msgCache reflete — null = cache non amorce
  const primeCache = () => {
    ...
    if (read.state === 'unreadable') { msgCache = null; cacheBytes = null; return new Map(); }
    msgCache = foldMessages(read.rows, deletedIds);
    cacheBytes = read.bytes;
    return msgCache;
  };
  /* Le fichier a-t-il bouge sous nous depuis l amorcage ? Si oui, un AUTRE processus a ecrit ou
   * compacte, et notre cache decrit un passe — on re-amorce plutot que de servir ce passe. */
  const readMsgs = () => {
    if (msgCache && cacheBytes != null) {
      let taille = null;
      try { taille = fs.statSync(file).size; } catch (e) { taille = null; }
      if (taille !== cacheBytes) { msgCache = null; cacheBytes = null; }
    }
    return [...(msgCache || primeCache()).values()];
  };
  ...
  function compact({ now } = {}) {
    ...
    cacheBytes = Buffer.byteLength(body, 'utf8');               // le cache reflete desormais CE fichier
  ...
  return {
    ...
    record(env, meta = {}) {
      ...
      const ligne = JSON.stringify(row) + '\n';
      fs.appendFileSync(file, ligne);
      /* Nos PROPRES ecrits suivent le compteur, sinon la garde de fraicheur ci-dessus verrait un ecart
       * a chaque lecture et re-amorcerait tout le journal — exactement ce que ce cache evite. */
      if (cacheBytes != null) cacheBytes += Buffer.byteLength(ligne, 'utf8');
    ...
    deleteMsg(id) {
      ...
      const pierre = JSON.stringify({ id, deleted: true, at: Date.now() }) + '\n';
      fs.appendFileSync(file, pierre);
      if (cacheBytes != null) cacheBytes += Buffer.byteLength(pierre, 'utf8');   // meme raison que record()
