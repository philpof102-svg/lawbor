...
function createStore(file = FILE, controlFile, opts = {}) {
  ...
  function compact({ now } = {}) {
    ...
    if (maxMessages > 0 && rows.length > maxMessages) {
      // Thread-ATOMIC retention. The fold's unit of meaning is the thread, not the message: a job's
      // opening `help_wanted`/`offer` seeds it and every later bid/quote/settle is dropped by the fold
      // if that seed is missing (work.js: "a bid for a job we have not seen: ignore"). A naive
      // slice(newest N) can cut a still-live thread in half — the newest replies survive, the seed is
      // gone, and the WHOLE negotiation silently vanishes on read (no crash, no trace). So drop whole
      // threads, newest-active first, and never truncate one. (rows is sorted oldest→newest.)
      ...
      const threads = [...byThread.values()].sort((a, b) => b.last - a.last);   // newest-active first
      const kept = [];
      for (const t of threads) {
        // keep whole threads until the next would overflow; always keep the newest thread even if it
        // alone exceeds the cap (a live thread's tail is never orphaned to honour a soft bound).
        if (kept.length && kept.length + t.msgs.length > maxMessages) break;
        for (const m of t.msgs) kept.push(m);
      }
      kept.sort((a, b) => orderOf(a) - orderOf(b));
      rows = kept;
    }
