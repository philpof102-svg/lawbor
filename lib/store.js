...
function createStore(file = FILE, controlFile) {
  ...
  let msgCache = null;
  const readMsgs = () => {
    if (!msgCache) { msgCache = new Map(); for (const m of readAll(file)) msgCache.set(m.id, m); }
    return [...msgCache.values()];
  };
  ...
  return {
    ...
    record(env, meta = {}) {
      ...
      if (msgCache) msgCache.set(row.id, row);              // keep the in-memory index in sync
      return row;
    },
