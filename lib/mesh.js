...
function classifyUrl(url, opts = {}) {
  ...
  const exempt = loopbackOk && isLocal;
  if (literal) {
...
function createMesh(cfg = {}) {
  ...
  async function addPeer(addr, url, opts = {}) {
    ...
    const cls = classifyUrl(url, { allowInsecure, allowLoopback });
  ...
  for (const s of (Array.isArray(cfg.anchors) ? cfg.anchors : [])) {
    ...
    const cls = classifyUrl(s.url, { allowInsecure, allowLoopback });
...
module.exports = { isLoopback, createMesh, classifyUrl, isPrivateAddress, normalizeIpv4 };
