...
function createMesh(cfg = {}) {
  ...
  const admitProbation = cfg.admitProbation === true;   // opt-in onboarding policy — see lib/relay.js
  ...
  async function gate(addr) {
    ...
    try {
      ...
      if (r && r.decision === 'PROCEED' && Number(r.score) >= minScore) {
        v = { ok: true, score: r.score, probation: false };
      } else if (admitProbation) {
        // See relay.js: probation is an opt-in operator policy. A probationer joins the mesh with score
        // 0 and the flag attached — they may speak, they earn nothing (conservation), and consent still
        // quarantines them. It buys nothing but the ability to start.
        v = { ok: true, score: 0, probation: true, decision: (r && r.decision) || 'UNKNOWN' };
      } else if (!r || r.decision !== 'PROCEED') {
  ...
  function tracesToAnchor(brut, seen) {
    /* Le contrat de la table (ligne ~340) dit addr(lower). Cinq accesseurs le respectent; celui-ci
     * lisait BRUT et ne marchait que parce que ses deux appelants passent des cles issues de book.
     * On applique le contrat ici aussi: une garde qui depend de son appelant nest pas une garde. */
    const addr = lower(brut);
