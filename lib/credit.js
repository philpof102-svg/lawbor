...
function creditFor(viewer, edges = [], opts = {}) {
  ...
  /* INBOUND — what each address has verifiably paid US. A different question from `direct`, and the
   * right one for the opposite role: `direct` answers "have I paid this WORKER before" (am I hiring a
   * proven hand), inbound answers "has this REQUESTER ever actually paid me" (is this client good for
   * it). Running the two-node demo is what exposed the difference — a worker was quoting a
   * stranger-premium to a client who had just paid him, because it was reading the wrong direction.
   * It is farm-resistant for the same conservation reason: it counts only money THEY irrecoverably
   * spent on US, which no third party and no ring can manufacture on their behalf. */
  const inbound = new Map();      // payer -> micro they paid V
  for (const x of pairs) {
    if (x.worker !== V || x.payer === V) continue;
    inbound.set(x.payer, (inbound.get(x.payer) || 0) + x.net);
  }
  ...
  return { direct, circle, inbound, evidence, netted: rf ? 'with-return-flow' : 'settlements-only', limits };
}

/**
 * explainCredit — the SAME fold, made auditable for ONE subject. A rating is only actionable when it
 * ships with WHY: this returns V's standing for W plus the exact evidence — every settlement txHash that
 * contributed, the depth-2 grants and which seed conferred them, and the conservation bound that binds.
 * Pure/deterministic (identical ordering to creditFor), zero network, no LLM. This is LAWBOR's pitch made
 * checkable: not "trust the score" but "here is every on-chain fact under it, verify them yourself".
 *
 * @returns {{
 *   viewer, subject,
 *   directMicro, circleMicro, inboundMicro, totalMicro,      // the numbers creditFor would give for W
 *   direct:  Array<{txHash,jobId,amountMicro,blockTime}>,    // V paid W — the raw edges, netted into directMicro
 *   circle:  Array<{via, grantMicro, viaSeededMicro}>,       // seeds V paid that conferred budget onto W
 *   inbound: Array<{txHash,jobId,amountMicro,blockTime}>,    // W paid V
 *   bound:   {spendMicro, alpha, maxTotalMicro, note},       // total ≤ (1+α)·spend(V), by construction
 *   netted, limits
 * }}
 */
function explainCredit(viewer, subject, edges = [], opts = {}) {
  const V = lower(viewer), W = lower(subject);
  const alpha = Number.isFinite(Number(opts.alpha)) ? Number(opts.alpha) : 0.5;
  const rf = opts.returnFlow || null;
  const back = (payee, payer) => { if (!rf) return 0; const k = payee + '|' + payer; return micro(rf.get ? rf.get(k) : rf[k]); };

  // pairs, netted — identical to creditFor so the explained numbers MATCH the rating exactly
  const pair = new Map();
  for (const e of edges) {
    const p = lower(e.payer), w = lower(e.worker); if (!p || !w) continue;
    const k = p + '|' + w;
  ...
  const pairs = [...pair.values()].map((x) => ({ ...x, net: Math.max(0, x.gross - back(x.worker, x.payer)) })).filter((x) => x.net > 0);

  // spend(V) — the bound's denominator: everything V irrecoverably paid out (netted)
  let spendMicro = 0;
  const budget = new Map();
  ...
  // depth-2 conferral, in the SAME deterministic order as creditFor, recording only what lands on W
  ...
  for (const x of ordered) {
    ...
    const grant = Math.min(x.net, b);
    if (grant <= 0) continue;
    budget.set(x.payer, b - grant);
    if (x.worker === W) { circleMicro += grant; circleEvidence.push({ via: x.payer, grantMicro: String(grant), viaSeededMicro: String(direct.get(x.payer) || 0) }); }
  }

  // raw edge evidence for the two DIRECT legs (V↔W), sorted by blockTime then txHash (as creditFor's evidence)
  const pick = (pred) => edges.filter(pred)
    .sort((a, b) => (Number(a.blockTime) || 0) - (Number(b.blockTime) || 0) || String(a.txHash).localeCompare(String(b.txHash)))
    .map((e) => ({ txHash: e.txHash, jobId: e.jobId, amountMicro: String(e.amountMicro), blockTime: e.blockTime }));
  const directEdges = pick((e) => lower(e.payer) === V && lower(e.worker) === W);
  const inboundEdges = pick((e) => lower(e.worker) === V && lower(e.payer) === W);

  const directMicro = direct.get(W) || 0;
  let inboundMicro = 0;
  for (const x of pairs) if (x.worker === V && x.payer === W) inboundMicro += x.net;

  const limits = [];
  if (!rf) limits.push('return-leg netting is OFF — a payee refunding by plain transfer is invisible here');
  if (V === W) limits.push('viewer and subject are the same address — a node confers no standing on itself');
  return {
    viewer: V, subject: W,
    directMicro: String(directMicro), circleMicro: String(circleMicro), inboundMicro: String(inboundMicro),
    totalMicro: String(directMicro + circleMicro),
    direct: directEdges, circle: circleEvidence, inbound: inboundEdges,
    bound: { spendMicro: String(spendMicro), alpha, maxTotalMicro: String(Math.floor(spendMicro * (1 + alpha))),
      note: 'standing V can see for ANYONE is bounded by (1+alpha)*spend(V), conserved and debited — a ring cannot manufacture it on V\'s behalf' },
    netted: rf ? 'with-return-flow' : 'settlements-only', limits,
  };
}

module.exports = { creditFor, explainCredit };
