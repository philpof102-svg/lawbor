 ...
 *
 * Pass `wired` (the node's ACTUAL relay.authenticates) whenever you have it. Without it this probes the
 * disk, and the disk is not the running process: a node booted before viem was installed reports
 * `available:true` (loadViem succeeds NOW) next to `authenticatesSenders:false` (it was wired at boot),
 * leaving a reader with two true facts and no way to reconcile them. Observed on a live node, and it is
 * the same class of defect as `delivered` — a status describing something other than what it names.
 */
function verifierStatus(wired) {
  const viem = loadViem();
  if (viem && wired === false) {
    return { available: true, via: 'viem', wired: false,
      note: 'viem IS installed, but THIS node is not authenticating: it was either started before viem was available (restart to pick it up) or told not to via LAWBOR_ALLOW_UNAUTHENTICATED=1, which accepts a spoofable `from`.' };
  }
  return viem
