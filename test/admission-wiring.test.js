...
t('★ sans LAWBOR_ADMISSION: rien ne change, et la raison annonce que operatedByUs restera true', () => {
  for (const env of [{}, { LAWBOR_ADMISSION: '' }, { LAWBOR_ADMISSION: '   ' }]) {
...
t('★ gitlawb SANS magasin d attestations: REFUS, et la raison dit que personne n entrerait', () => {
  const p = planAdmissionWiring({ LAWBOR_ADMISSION: 'gitlawb', GITLAWB_NODE: 'https://n' });
...
t('★ gitlawb SANS noeud: REFUS — aucun defaut, parce que le choix du noeud EST la decision', () => {
  const p = planAdmissionWiring({ LAWBOR_ADMISSION: 'gitlawb', LAWBOR_BINDINGS: '/tmp/b.jsonl' });
  assert.strictEqual(p.ok, false);
...
t('une valeur INCONNUE est refusee, jamais ignoree en silence', () => {
  const p = planAdmissionWiring({ LAWBOR_ADMISSION: 'quelquechose' });
  assert.strictEqual(p.ok, false);
...
t('★ TEMOIN — les deux variables posees: le plan passe en mode gitlawb', () => {
  const p = planAdmissionWiring({ LAWBOR_ADMISSION: 'gitlawb',
...
t('★ TEMOIN 2 — des environnements differents donnent des plans differents', () => {
  ...
  const b = planAdmissionWiring({ LAWBOR_ADMISSION: 'gitlawb', GITLAWB_NODE: 'https://n',
...
t('★ le message de refus nomme CE QUI MANQUE et comment revenir en arriere', () => {
  ...
  assert.ok(/retirer LAWBOR_ADMISSION/.test(m),
