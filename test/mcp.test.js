...
(async () => {
  ...
  await t('initialize + tools/list expose the 18 lawbor tools on protocol ' + PROTOCOL, async () => {
    ...
    assert.equal(list.result.tools.map((x) => x.name).sort().join(),
      'lawbor_accept,lawbor_award,lawbor_bid,lawbor_block,lawbor_bot_say,lawbor_credit,lawbor_graph,lawbor_inbox,lawbor_jobs,lawbor_post_job,lawbor_requests,lawbor_say,lawbor_settle,lawbor_thread,lawbor_unblock,lawbor_wanted,lawbor_watch,lawbor_whoami');
  });
