  ...
  switch (method) {
    ...
    case 'tools/call': {
      ...
      try {
        ...
        if (name === 'lawbor_whoami') {
        ...
        } else if (name === 'lawbor_post_job' || name === 'lawbor_bid' || name === 'lawbor_award' || name === 'lawbor_settle' || name === 'lawbor_validate' || name === 'lawbor_offer' || name === 'lawbor_quote') {
          const kind = name === 'lawbor_post_job' ? 'help_wanted' : name === 'lawbor_bid' ? 'bid'
            : name === 'lawbor_settle' ? 'settle' : name === 'lawbor_validate' ? 'validate' : name === 'lawbor_offer' ? 'offer' : name === 'lawbor_quote' ? 'quote' : 'award';
          ...
          if (kind === 'quote' || kind === 'settle') {
            const jb = work.foldThread(node.store.all()).get(a.jobId);
            if (kind === 'quote') payload.agreedPrice = (jb && jb.agreedPrice) || null;
            if (kind === 'settle' && jb && jb.agreedPrice && String(jb.agreedPrice.amountMicro) !== String(a.amountMicro)) {
              payload.warnings = (payload.warnings || []).concat('settle amountMicro ' + a.amountMicro + ' ≠ agreedPrice ' + jb.agreedPrice.amountMicro + ' — advisory only, the settle is not blocked.');
            }
          }
