...
const TOOLS = [
  ...
  { name: 'lawbor_validate', description: 'Record a counterparty\'s proof of KEY-CONTROL, bound to a jobId: evidence that an address actually holds its key, so a payment is not sent to an address nobody controls. Two equivalent methods: (a) keyAddr + keySig — an off-chain signature over the exact string "LAWBOR-KEY:<lowercased addr>", requiring no gas and no USDC (an empty wallet can still prove it); or (b) txHash of a USDC transfer on Base signed BY that address — used when the fact needs a public on-chain timestamp, or when the address is a smart-contract wallet that cannot sign a message. A signature BY the address proves key-control; a transfer merely TO an address does not. A validation confers zero reputation/standing. Input: to, jobId, and EITHER txHash OR (keyAddr + keySig).', inputSchema: { type: 'object', properties: { to: { type: 'string' }, jobId: { type: 'string' }, txHash: { type: 'string' }, keyAddr: { type: 'string' }, keySig: { type: 'string' }, as: { type: 'string' } }, required: ['to', 'jobId'] } },
  ...
  { name: 'lawbor_settle', description: 'Bind an awarded job to a REAL USDC transfer on Base, by txHash. Only the requester or the awarded worker may settle, and only after an award. The claim becomes a verified settlement ONLY if the on-chain tx matches the signed award exactly (Base chainId 8453, USDC, payer = the requester who signed the award, payee = the awarded worker, exact amount, >=12 confirmations); otherwise it confers nothing. LAWBOR moves no funds — the two parties transfer directly and this records a pointer anyone can re-verify and refute. `settled` means PAID, never delivered — unless the award committed a deliverableHash, in which case pass receivedHash (sha256 of what you actually got) and this returns a SECOND, INDEPENDENT verdict: served | substituted | unverifiable. Independent is the point: `verified` means the money moved and stays true whatever arrived, because a chain does not un-transfer USDC when a buyer is unhappy — a verified payment beside a substituted delivery is exactly what a dispute needs described rather than collapsed. `unverifiable` is the honest answer when the award committed to nothing, which is every award written before this existed. Input: to, jobId, txHash (0x, 32 bytes), amountMicro (USDC 6-decimals, integer string), deliverable? (an opaque, unverified pointer), receivedHash?.', inputSchema: { type: 'object', properties: { to: { type: 'string' }, jobId: { type: 'string' }, txHash: { type: 'string' }, amountMicro: { type: 'string' }, deliverable: { type: 'string' }, receivedHash: { type: 'string', description: '0x + sha256 of the bytes you actually received, compared against the hash the award committed to.' }, as: { type: 'string' } }, required: ['to', 'jobId', 'txHash', 'amountMicro'] } },
  ...
  { name: 'lawbor_rings', description: 'READ-ONLY: the STRUCTURAL anti-farming lens — short directed CYCLES in this node\'s verified settlement graph, i.e. addresses that paid each other in a loop so the money returned to where it started (the on-chain signature of wash / self-dealing). Returns each cycle\'s members, hop count, and `recycledMicro` (the bottleneck that provably circulated), plus totalRecycledMicro and addressesInRings. ADVISORY only: it never enters the rating (which is already conservation-bounded and unfarmable per viewer) and never gates admission — a legitimate mutual-trade loop can also form a cycle. Short cycles only (bounded, total). No input.', inputSchema: { type: 'object', properties: {}, additionalProperties: false } },
  { name: 'lawbor_why', description: 'READ-ONLY: WHY this node\'s standing for an address is the number it is — the rating made auditable, provable per txHash. Returns directMicro/circleMicro/inboundMicro/totalMicro (matching lawbor_credit exactly) PLUS the evidence trail: `direct` = the raw settlement edges this node paid them (each with txHash + jobId), `circle` = the depth-2 grants with which SEED conferred each one, `inbound` = what they paid this node, and `bound` = the conservation ceiling total ≤ (1+alpha)·spend(viewer) that makes it unfarmable. A rating you can verify on Base yourself, not one you must trust. Input: of (0x address).', inputSchema: { type: 'object', properties: { of: { type: 'string' } }, required: ['of'], additionalProperties: false } },
  { name: 'lawbor_award', description: 'Award your job to one bidder. Only the requester may award, and the award restates the agreed price — that is the requester\'s signed commitment. Settlement is NOT included: settlementRef is an opaque string LAWBOR never creates or checks. COMMIT TO WHAT YOU WILL DELIVER: pass deliverableHash (0x + sha256 of the exact bytes) and the buyer can later PROVE it received what was promised. This rail has always said `settled` means PAID, never delivered — a payment is a fact on Base anyone can re-check forever, while the deliverable was a sentence in a message. A hash committed HERE closes half of that: the award must precede any settle, so the commitment is structurally BEFORE the money, which proves the deliverable existed first (you cannot hash what you have not made) and was not swapped after the funds cleared. It proves NOTHING about quality — a committed hash of garbage verifies perfectly. The rail carries the digest, never the artifact. Input: to (the winner), jobId, worker, price, eta?, settlementRef?, deliverableHash?.', inputSchema: { type: 'object', properties: { to: { type: 'string' }, jobId: { type: 'string' }, worker: { type: 'string' }, price: { type: 'string' }, eta: { type: 'string' }, settlementRef: { type: 'string' }, deliverableHash: { type: 'string', description: '0x + sha256 of the exact deliverable bytes, committed BEFORE payment is permitted.' } }, required: ['to', 'jobId', 'worker', 'price'] } },
  ...
  switch (method) {
    ...
    case 'tools/call': {
      ...
      try {
        ...
        } else if (name === 'lawbor_rings') {
          // Structural anti-farming lens: short settlement cycles (money that came back). Advisory,
          // never enters the rating. Same verified edges the credit fold uses.
          const { blocked: rBlocked } = node.store.control();
          const rMsgs = node.store.all().filter((m) => !rBlocked.has(String(m.from).toLowerCase()));
          if (typeof deps.resolveFacts === 'function') await deps.resolveFacts(rMsgs);
          const edges = work.settlementsFrom(rMsgs, { txFacts: deps.txFacts || null });
          payload = detectRings(edges);
          if (!deps.txFacts) payload.limits = (payload.limits || []).concat('no chain reader is wired here, so NO settlement can verify and the graph is empty — this is not evidence of no rings');
        } else if (name === 'lawbor_why') {
          // WHY the standing is what it is — the rating made auditable for ONE subject, provable per txHash.
          // Same fold as lawbor_credit, instrumented: every contributing settlement edge, the depth-2 grants
          // and which seed conferred them, and the conservation bound that binds. A rating you can verify
          // yourself beats a rating you must trust — that is LAWBOR's whole pitch, made checkable.
          if (!a.of) { payload = { error: 'lawbor_why needs `of` — the address to explain THIS node\'s standing for' }; }
          else {
            const { blocked: yBlocked } = node.store.control();
            const yMsgs = node.store.all().filter((m) => !yBlocked.has(String(m.from).toLowerCase()));
            if (typeof deps.resolveFacts === 'function') await deps.resolveFacts(yMsgs);
            const edges = work.settlementsFrom(yMsgs, { txFacts: deps.txFacts || null });
            payload = explainCredit(node.self, String(a.of).toLowerCase(), edges, { returnFlow: deps.returnFlow || null });
            payload.limits = (payload.limits || []).concat([
              'this is THIS node\'s view only — no global score exists, and two nodes will disagree by design',
            ], deps.txFacts ? [] : ['no chain reader is wired here, so NO settlement can verify and every number is 0 — this is not evidence of a bad counterparty']);
          }
