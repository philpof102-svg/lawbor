...
const TOOLS = [
  ...
  { name: 'lawbor_validate', description: 'THE PENNY-DROP: cite a ZERO-value (or dust) USDC transfer on Base to prove the payment RAIL works before real money crosses it — the bank micro-deposit, on-chain. Use it BEFORE settling a real amount. DIRECTION IS THE PROOF: a tx you send TO the worker proves only that your key works and the address accepts transfers; a tx signed BY the worker proves THEY control that address, which is the question that matters before you pay them. This is the cheapest defence against the one irreversible loss in the system: paying an address nobody controls. A validation NEVER becomes reputation — it costs only gas, so it is farmable by construction and confers exactly zero standing. Input: to, jobId, txHash.', inputSchema: { type: 'object', properties: { to: { type: 'string' }, jobId: { type: 'string' }, txHash: { type: 'string' }, as: { type: 'string' } }, required: ['to', 'jobId', 'txHash'] } },
